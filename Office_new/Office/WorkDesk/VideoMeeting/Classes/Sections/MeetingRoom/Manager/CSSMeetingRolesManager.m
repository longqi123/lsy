...
#import "CSSMeetingMessageHandler.h"
...
@interface CSSMeetingRolesManager()<CSSMeetingMessageHandlerDelegate, CSSTimerHolderDelegate>

@property(nonatomic, strong) NIMChatroom *chatroom;

@property(nonatomic, strong) NSMutableDictionary *meetingRoles;

@property(nonatomic, strong) CSSMeetingMessageHandler *messageHandler;
...
@implementation CSSMeetingRolesManager
             ...
             newCreated:(BOOL)newCreated
{
    ...
    _messageHandler = [[CSSMeetingMessageHandler alloc] initWithChatroom:chatroom delegate:self];
    ...
    if (!newCreated) {
        CSSTimerHolder *timerHolder = [[CSSTimerHolder alloc] init];
...
{
    CSSMeetingRole *role = [self role:member.userId];
    if (!role) {
...
{
    CSSMeetingRole *role = [self myRole];
...
- (NSArray *)allActors
{
    ...
    for (CSSMeetingRole *role in _meetingRoles.allValues) {
...
{
    CSSMeetingRole *myRole = [self myRole];
...
{
    CSSMeetingRole *role = [self role:user];
...
- (void)updateVolumes:(NSDictionary<NSString *, NSNumber *> *)volumes
{
    for (NSString *meetingUser in _meetingRoles.allKeys) {
        ...
        CSSMeetingRole *role = [self role:meetingUser];
        role.audioVolume = volume;
    }
    [self notifyMeetingVolumesUpdate];
}

#pragma mark - CSSMeetingMessageHandlerDelegate
...
- (CSSMeetingRole *)addNewRole:(NSString *)uid asActor:(BOOL)actor
{
    DDLogInfo(@"Add new role : %@, is actor : %@", uid, actor ? @"YES" : @"NO");
    CSSMeetingRole *newRole = [[CSSMeetingRole alloc] init];
...
- (void)updateRolesFromManager:(NSArray *)actorsMember
{
    ...
    for (NSString *actorId in actorsMember) {
        CSSMeetingRole *role = [self role:actorId];
        if (!role) {
...
- (CSSMeetingControlAttachment *)actorsListAttachment
{
    CSSMeetingControlAttachment *attachment = [[CSSMeetingControlAttachment alloc] init];
