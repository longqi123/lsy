//
//  CSSMeetingWhiteboardViewController.m
//  NIMEducationDemo
//
...
#import "CSSColorSelectView.h"
#import "CSSMeetingRTSManager.h"
#import "UIView+Toast.h"
#import "CSSWhiteboardCommand.h"
#import "CSSWhiteboardCmdHandler.h"
#import "CSSWhiteboardLines.h"
#import "CSSWhiteboardDrawView.h"
#import "CSSDocumentViewController.h"
#import "UIAlertView+CSSBlock.h"
#import "CSSDocumentShareInfo.h"
#import "CSSDocumentHandler.h"
#import "UIImageView+WebCache.h"
#import "CSSDocDownloadManager.h"


@interface CSSMeetingWhiteboardViewController ()<CSSColorSelectViewDelegate, CSSMeetingRTSManagerDelegate, CSSWhiteboardCmdHandlerDelegate, NIMLoginManagerDelegate,CSSDocumentHandlerDelegate>
{
    BOOL _isManager;
    NSString *_name;
...
@property (nonatomic, strong) CSSWhiteboardDrawView *drawView;
...
@property (nonatomic, strong) CSSColorSelectView *colorSelectView;

@property (nonatomic, strong) UILabel *hintLabel;

@property (nonatomic, strong) CSSWhiteboardCmdHandler *cmdHander;

@property (nonatomic, strong) CSSDocumentHandler *docHander;

@property (nonatomic, strong) CSSWhiteboardLines *lines;
...
@implementation CSSMeetingWhiteboardViewController
...
- (instancetype)initWithChatroom:(NIMChatroom *)room
{
    if (self = [super init]) {
        ...
        _cmdHander = [[CSSWhiteboardCmdHandler alloc] initWithDelegate:self];
        _docHander = [[CSSDocumentHandler alloc]initWithDelegate:self];
        [[CSSMeetingRTSManager sharedInstance] setDataHandler:_cmdHander];
        ...
        _lines = [[CSSWhiteboardLines alloc] init];
...
- (void)dealloc
{
    [[CSSMeetingRTSManager sharedInstance] leaveCurrentConference];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    [[NIMSDK sharedSDK].loginManager addDelegate:self];
    _isManager = [CSSMeetingRolesManager sharedInstance].myRole.isManager;
    [[CSSMeetingRTSManager sharedInstance] setDelegate:self];
    
    NSError *error;
    if (_isManager) {
        error = [[CSSMeetingRTSManager sharedInstance] reserveConference:_name];
    }
    else {
        error = [[CSSMeetingRTSManager sharedInstance] joinConference:_name];
    }
...
{
    if ([CSSMeetingRolesManager sharedInstance].myRole.whiteboardOn && _isJoined) {
...
{
    if (!_drawView) {
        _drawView = [[CSSWhiteboardDrawView alloc] initWithFrame:CGRectZero];
...
- (CSSColorSelectView *)colorSelectView
{
    if (!_colorSelectView) {
        _colorSelectView = [[CSSColorSelectView alloc] initWithFrame:CGRectZero
                                                               colors:_colors
                                                             delegate:self];
    }
...
{
    CSSDocumentViewController *docVc = [[CSSDocumentViewController alloc]init];
    docVc.delegate = self;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self onPointCollected:p type:CSSWhiteboardPointTypeStart];
}

- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    CGPoint p = [[touches anyObject] locationInView:_drawView];
    [self onPointCollected:p type:CSSWhiteboardPointTypeMove];
}

- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    CGPoint p = [[touches anyObject] locationInView:_drawView];
    [self onPointCollected:p type:CSSWhiteboardPointTypeEnd];
}

- (void)onPointCollected:(CGPoint)p type:(CSSWhiteboardPointType)type
{
    if (!([CSSMeetingRolesManager sharedInstance].myRole.whiteboardOn)) {
        return;
    }
...
# pragma mark - CSSMeetingRTSManagerDelegate
- (void)onReserve:(NSString *)name result:(NSError *)result
{
    if (result == nil) {
        NSError *result = [[CSSMeetingRTSManager sharedInstance] joinConference:_name];
...
#pragma mark - CSSWhiteboardCmdHandlerDelegate
...
- (void)onReceiveSyncPoints:(NSMutableArray *)points owner:(NSString *)owner
{
    ...
    for (CSSWhiteboardPoint *point in points) {
...
#pragma mark - CSSDocumentHandlerDelegate
...
#pragma mark - CSSDocumentViewControllerDelegate
...
-(void)loadImageOnWhiteboard
{
    ...
    if (!image) {
        ...
        [[CSSDocDownloadManager sharedManager]downLoadDoc:self.docInfo page:_currentPage completeBlock:^(NSError *error) {
            if (!error) {
...
{
    NSString *filePath = [[CSSDocumentHandler getFilePathPrefix:self.docInfo.docId]stringByAppendingString:[NSString stringWithFormat:@"%@_%zd.png",self.docInfo.docName,pageNum]];
