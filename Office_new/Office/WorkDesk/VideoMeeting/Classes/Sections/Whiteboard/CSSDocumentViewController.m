...
@interface CSSDocumentViewController ()<CSSDocDownloadManagerDelegate>

@property (nonatomic,strong)UITableView *tableView;
@property (nonatomic,strong)CSSDocumentHandler *handler;
@property (nonatomic,strong)NSMutableArray<NIMDocTranscodingInfo*> *docInfos;
@end

#define cellReuseIdentifier @"documentCell"
@implementation CSSDocumentViewController
...
- (void)viewDidLoad {
    ...
    _handler = [[CSSDocumentHandler alloc]initWithDelegate:self];
    [_handler fetchMyDocsInfo:nil limit:30];
    
    [[CSSDocDownloadManager sharedManager]addDelegate:self];
}
...
-(UITableView*)tableView
{
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[CSSDocumentCell class] forCellReuseIdentifier:cellReuseIdentifier];
...
{
    CSSDocumentCell *cell = [tableView dequeueReusableCellWithIdentifier:cellReuseIdentifier];
    cell.delegate = self;
...
#pragma mark - CSSDocDownloadManagerDelegate
...
#pragma mark - CSSDocumentCellDelegate
...
{
    [[CSSDocDownloadManager sharedManager]downLoadDoc:info];
}
