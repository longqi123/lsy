...
@interface CSSpellingCenter ()
- (CSSpellingUnit *)calcSpellingOfString: (NSString *)source;
@end


@implementation CSSpellingCenter
+ (CSSpellingCenter *)sharedCenter
{
    static CSSpellingCenter *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[CSSpellingCenter alloc]init];
    });
    return instance;
}
...
- (CSSpellingUnit *)spellingForString:(NSString *)source
{
    if ([source length] == 0)
    {
        return nil;
    }
    CSSpellingUnit *spellingUnit = nil;
    @synchronized(self)
    {
        CSSpellingUnit *unit = [_spellingCache objectForKey:source];
        if (!unit)
        {
...
{
    CSSpellingUnit *unit = [self spellingForString:input];
...
- (CSSpellingUnit *)calcSpellingOfString:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[CSPinyinConverter sharedInstance] toPinyin:word];
    ...
    CSSpellingUnit *unit = [[CSSpellingUnit alloc]init];
