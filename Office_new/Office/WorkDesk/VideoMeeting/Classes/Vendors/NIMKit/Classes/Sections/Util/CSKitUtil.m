...
+ (NSString *)showNick:(NSString*)uid inMessage:(NIMMessage*)message
{
    ...
    return [[CSKit sharedKit] infoByUser:uid
...
+ (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    result = [CSKitUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12)
    {
        hour = hour - 12;
    }
    ...
    {
        NSString *weekDay = [CSKitUtil weekdayStr:msgDateComponents.weekday];
...
+ (NSString *)formatedMessage:(NIMMessage *)message{
    switch (message.messageType) {
        case NIMMessageTypeNotification:
            return [CSKitUtil notificationMessage:message];
...
+ (NSString *)notificationMessage:(NIMMessage *)message{
    ...
    switch (object.notificationType) {
        case NIMNotificationTypeTeam:{
            return [CSKitUtil teamNotificationFormatedMessage:message];
        }
        case NIMNotificationTypeNetCall:{
            return [CSKitUtil netcallNotificationFormatedMessage:message];
        }
        case NIMNotificationTypeChatroom:{
            return [CSKitUtil chatroomNotificationFormatedMessage:message];
        }
        default:
            return @"";
    }
...
+ (NSString*)teamNotificationFormatedMessage:(NIMMessage *)message{
    ...
    if (object.notificationType == NIMNotificationTypeTeam)
    {
        ...
        NSString *source = [CSKitUtil teamNotificationSourceName:message];
        NSArray *targets = [CSKitUtil teamNotificationTargetNames:message];
        NSString *targetText = [targets count] > 1 ? [targets componentsJoinedByString:@","] : [targets firstObject];
        NSString *teamName = [CSKitUtil teamNotificationTeamShowName:message];
...
+ (NSString *)teamNotificationSourceName:(NIMMessage *)message{
    ...
    if ([content.sourceID isEqualToString:currentAccount]) {
        ...
        source = [CSKitUtil showNick:content.sourceID inSession:message.session];
    }
    return source;
}
...
+ (NSArray *)teamNotificationTargetNames:(NIMMessage *)message{
    ...
    for (NSString *item in content.targetIDs) {
        if ([item isEqualToString:currentAccount]) {
            ...
            NSString *targetShowName = [CSKitUtil showNick:item inSession:message.session];
