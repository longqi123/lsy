...
#import "CSKitUtil.h"
#import "CSSessionAudioContentView.h"
#import "UIView+CS.h"
...
@interface CSMessageCell()<NIMPlayAudioUIDelegate,NIMMessageContentViewDelegate>{
    UILongPressGestureRecognizer *_longPressGesture;
    UIMenuController             *_menuController;
}

@property (nonatomic,strong) CSMessageModel *model;
...
- (void)makeComponents
{
    ...
    [_retryButton setImage:[UIImage cs_imageInKit:@"icon_message_cell_error"] forState:UIControlStateNormal];
    [_retryButton setImage:[UIImage cs_imageInKit:@"icon_message_cell_error"] forState:UIControlStateHighlighted];
    ...
    //audioPlayedIcon
    _audioPlayedIcon = [CSBadgeView viewWithBadgeTip:@""];
    ...
    //headerView
    _headImageView = [[CSAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    ...
    _nameLabel.font = UIFont.H9;
    [_nameLabel setTextColor:UIColor.T7];
...
- (BOOL)checkData{
    return [self.model isKindOfClass:[CSMessageModel class]];
}


- (void)setModel:(CSMessageModel *)model{
    _model = model;
}
...
- (void)refresh{
    ...
    {
        NSString *nick = [CSKitUtil showNick:self.model.message.from inMessage:self.model.message];
...
- (void)addContentViewIfNotExist
{
    if (_bubbleView == nil)
    {
        id<CSCellLayoutConfig> config = self.model.layoutConfig;
        NSString *contentStr = [config cellContent:self.model];
        if (!contentStr.length) {
            //针对上层实现了cellContent:接口，但是没有覆盖全的情况
            CSCellLayoutDefaultConfig *config = [CSDefaultValueMaker sharedMaker].cellLayoutDefaultConfig;
            contentStr = [config cellContent:self.model];
        }
        Class clazz = NSClassFromString(contentStr);
        CSSessionMessageContentView *contentView =  [[clazz alloc] initSessionMessageContentView];
        if (!contentView) {
        ...
        if (messageType == NIMMessageTypeAudio) {
            ((CSSessionAudioContentView *)_bubbleView).audioUIDelegate = self;
        }
...
- (CSSessionMessageContentView *)unSupportContentView{
    return [[CSSessionUnknowContentView alloc] initSessionMessageContentView];
}
