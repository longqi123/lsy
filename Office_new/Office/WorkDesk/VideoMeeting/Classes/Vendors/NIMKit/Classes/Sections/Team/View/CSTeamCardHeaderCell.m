...
@interface CSTeamCardHeaderCell()

@property (nonatomic,strong) id<CSKitCardHeaderData> data;

@end

@implementation CSTeamCardHeaderCell

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        _imageView                  = [[CSAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 55, 55)];
...
- (void)refreshData:(id<CSKitCardHeaderData>)data{
    ...
    if([data isKindOfClass:[CSTeamCardMemberItem class]]) {
        CSTeamCardMemberItem *member = data;
