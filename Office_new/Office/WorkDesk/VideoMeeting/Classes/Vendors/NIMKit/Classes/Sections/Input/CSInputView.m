...
#import "CSInputToolBar.h"
#import "UIImage+CS.h"
#import "CSUIConfig.h"
#import "CSGlobalMacro.h"


@interface CSInputView()<UITextViewDelegate,CSInputEmoticonProtocol>
{
...
@property (nonatomic, strong) CSInputAudioRecordIndicatorView *audioRecordIndicator;
@property (nonatomic, assign) NIMAudioRecordPhase recordPhase;
@property (nonatomic, weak) id<CSSessionConfig> inputConfig;
...
- (CSInputAudioRecordIndicatorView *)audioRecordIndicator {
    if(!_audioRecordIndicator) {
        _audioRecordIndicator = [[CSInputAudioRecordIndicatorView alloc] init];
    }
...
- (void)initUIComponents
{
    ...
    _toolBar = [[CSInputToolBar alloc] initWithFrame:CGRectZero];
    ...
    _toolBar.inputTextView.returnKeyType = UIReturnKeyDone;
    ...
    _inputTextViewOlderHeight = [CSUIConfig topInputViewHeight];
    //[_toolBar.recordButton setHidden:YES];
    [self addListenEvents];
    
    
    //_moreContainer = [[CSInputMoreContainerView alloc] initWithFrame:CGRectMake(0, [CSUIConfig topInputViewHeight], self.nim_width, [CSUIConfig bottomInputViewHeight])];
    //_moreContainer.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    //_moreContainer.hidden   = YES;
    //[self addSubview:_moreContainer];
    
    //_emoticonContainer = [[CSInputEmoticonContainerView alloc] initWithFrame:CGRectMake(0, [CSUIConfig topInputViewHeight], self.nim_width, [CSUIConfig bottomInputViewHeight])];
...
{
    return CSKit_IOS8 ? textView.contentSize.height : ceilf([textView sizeThatFits:textView.frame.size].height);
}

- (void)updateVoiceBtnImages:(BOOL)selected
{
    /*
    [_toolBar.voiceBtn setImage:selected?[UIImage cs_imageInKit:@"icon_toolview_voice_normal"]:[UIImage cs_imageInKit:@"icon_toolview_keyboard_normal"] forState:UIControlStateNormal];
    [_toolBar.voiceBtn setImage:selected?[UIImage cs_imageInKit:@"icon_toolview_voice_pressed"]:[UIImage cs_imageInKit:@"icon_toolview_keyboard_pressed"] forState:UIControlStateHighlighted];
     */
}
...
{
    /*
    [_toolBar.emoticonBtn setImage:selected?[UIImage cs_imageInKit:@"icon_toolview_emotion_normal"]:[UIImage cs_imageInKit:@"icon_toolview_keyboard_normal"] forState:UIControlStateNormal];
    [_toolBar.emoticonBtn setImage:selected?[UIImage cs_imageInKit:@"icon_toolview_emotion_pressed"]:[UIImage cs_imageInKit:@"icon_toolview_keyboard_pressed"] forState:UIControlStateHighlighted];
     */
}
...
{
    toHeight = MAX([CSUIConfig topInputViewHeight], toHeight);
    toHeight = MIN([CSUIConfig bottomInputViewHeight], toHeight);
            ...
            [[AVAudioSession sharedInstance] performSelector:@selector(requestRecordPermission:) withObject:^(BOOL granted) {
                if (granted) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [self inputTextViewToHeight:[CSUIConfig topInputViewHeight]];;
...
- (void)onTouchEmoticonBtn:(id)sender
{
    ...
    if (_inputType != InputTypeEmot) {
        ...
        _inputBottomViewHeight = [CSUIConfig bottomInputViewHeight];
...
- (void)selectedEmoticon:(NSString*)emoticonID catalog:(NSString*)emotCatalogID description:(NSString *)description{
    if (!emotCatalogID) { //删除键
        ...
        if ([emotCatalogID isEqualToString:CSKit_EmojiCatalog]) {
...
- (NSRange)rangeForEmoticon
{
    ...
    else
    {
        ...
        CSInputEmoticon *icon = [[CSInputEmoticonManager sharedManager] emoticonByTag:name];
