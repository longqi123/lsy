...
#import "CSAttributedLabelAttachment.h"
...
@property (nonatomic,strong)    CSAttributedLabelURL *touchedLink;
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [_attributedString cs_setFont:_font];
        [self resetFont];
        for (CSAttributedLabelAttachment *attachment in _attachments)
        {
...
- (void)setTextColor:(UIColor *)textColor
{
    if (textColor && _textColor != textColor)
    {
        ...
        [_attributedString cs_setTextColor:textColor];
        [self resetTextFrame];
    }
}
...
- (NSAttributedString *)attributedString:(NSString *)text
{
    if ([text length])
    {
        ...
        [string cs_setFont:self.font];
        [string cs_setTextColor:self.textColor];
        return string;
    }
    else
    {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (CSAttributedLabelURL *url in _linkLocations)
        {
            ...
            [drawString cs_setTextColor:drawLinkColor range:url.range];
            [drawString cs_setUnderlineStyle:_underLineForLink ? kCTUnderlineStyleSingle : kCTUnderlineStyleNone
...
- (CSAttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            CSAttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    CSAttributedLabelURL *url = [self urlForPoint:point];
...
- (CSAttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (CSAttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (CSAttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
...
{
    CSAttributedLabelAttachment *attachment = [CSAttributedLabelAttachment attachmentWith:image
                                                                                     margin:margin
                                                                             alignment:alignment
                                                                               maxSize:maxSize];
...
- (void)appendView: (UIView *)view
         ...
         alignment: (NIMImageAlignment)alignment
{
    CSAttributedLabelAttachment *attachment = [CSAttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
{
    CSAttributedLabelURL *url = [CSAttributedLabelURL urlWithLinkData:linkData
                                                                  range:range
                                                                  color:color];
...
{
    [CSAttributedLabelURL setCustomDetectMethod:block];
}
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            CSAttributedLabelAttachment* attributedImage = (CSAttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (BOOL)onLabelClick:(CGPoint)point
{
    ...
    if (linkData)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(CSAttributedLabel:clickedOnLink:)])
        {
            [_delegate CSAttributedLabel:self clickedOnLink:linkData];
        }
        else
        {
            NSURL *url = nil;
               ...
               sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if ([links count])
        {
            for (CSAttributedLabelURL *link in links)
            {
    ...
    {
        _ignoreRedraw = YES;
        NSArray *links = [CSAttributedLabelURL detectLinks:text];
        block(links);
        _ignoreRedraw = NO;
    }
    else
...
- (void)addAutoDetectedLink: (CSAttributedLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    CSAttributedLabelURL *touchedLink = [self urlForPoint:point];
