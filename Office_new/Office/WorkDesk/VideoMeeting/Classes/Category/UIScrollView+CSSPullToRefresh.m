//
// UIScrollView+CSSPullToRefresh.m
//
...
#import "UIScrollView+CSSPullToRefresh.h"

#define fequal(a,b) (fabs((a) - (b)) < FLT_EPSILON)
#define fequalzero(a) (fabs(a) < FLT_EPSILON)

static CGFloat const CSSPullToRefreshViewHeight = 60;

@interface CSSPullToRefreshArrow : UIView
...
@interface CSSPullToRefreshView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) CSSPullToRefreshArrow *arrow;
@property (nonatomic, strong) UIActivityIndicatorView *activityIndicatorView;
@property (nonatomic, strong, readwrite) UILabel *titleLabel;
@property (nonatomic, strong, readwrite) UILabel *subtitleLabel;
@property (nonatomic, readwrite) CSSPullToRefreshState state;
@property (nonatomic, readwrite) CSSPullToRefreshPosition position;
...
#pragma mark - UIScrollView (CSSPullToRefresh)
#import <objc/runtime.h>

static char UIScrollViewPullToRefreshView;

@implementation UIScrollView (CSSPullToRefresh)
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(CSSPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case CSSPullToRefreshPositionTop:
                yOrigin = -CSSPullToRefreshViewHeight;
                break;
            case CSSPullToRefreshPositionBottom:
                yOrigin = self.contentSize.height;
                break;
            default:
                return;
        }
        CSSPullToRefreshView *view = [[CSSPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, CSSPullToRefreshViewHeight)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    [self addPullToRefreshWithActionHandler:actionHandler position:CSSPullToRefreshPositionTop];
}

- (void)triggerPullToRefresh {
    self.pullToRefreshView.state = CSSPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)setPullToRefreshView:(CSSPullToRefreshView *)pullToRefreshView {
...
- (CSSPullToRefreshView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, CSSPullToRefreshViewHeight);
        }
    }
}
...
#pragma mark - CSSPullToRefresh
@implementation CSSPullToRefreshView
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.state = CSSPullToRefreshStateStopped;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            case CSSPullToRefreshStateAll:
            case CSSPullToRefreshStateStopped:
                self.arrow.alpha = 1;
            ...
            case CSSPullToRefreshStateTriggered:
                switch (self.position) {
                    case CSSPullToRefreshPositionTop:
                        [self rotateArrow:(float)M_PI hide:NO];
                        break;
            ...
            case CSSPullToRefreshStateLoading:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, CSSPullToRefreshViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != CSSPullToRefreshStateLoading) {
        ...
        if(!self.scrollView.isDragging && self.state == CSSPullToRefreshStateTriggered)
            self.state = CSSPullToRefreshStateLoading;
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == CSSPullToRefreshStateStopped && self.position == CSSPullToRefreshPositionTop)
            self.state = CSSPullToRefreshStateTriggered;
        else if(contentOffset.y >= scrollOffsetThreshold && self.state != CSSPullToRefreshStateStopped && self.position == CSSPullToRefreshPositionTop)
            self.state = CSSPullToRefreshStateStopped;
        else if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == CSSPullToRefreshStateStopped && self.position == CSSPullToRefreshPositionBottom)
            self.state = CSSPullToRefreshStateTriggered;
        else if(contentOffset.y <= scrollOffsetThreshold && self.state != CSSPullToRefreshStateStopped && self.position == CSSPullToRefreshPositionBottom)
            self.state = CSSPullToRefreshStateStopped;
    } else {
        CGFloat offset;
...
- (CSSPullToRefreshArrow *)arrow {
    if(!_arrow) {
        _arrow = [[CSSPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (void)setTitle:(NSString *)title forState:(CSSPullToRefreshState)state {
    ...
    if(state == CSSPullToRefreshStateAll)
...
- (void)setState:(CSSPullToRefreshState)newState {
    
    if(_state == newState)
        return;
    
    CSSPullToRefreshState previousState = _state;
    _state = newState;
    ...
    switch (newState) {
        ...
        case CSSPullToRefreshStateLoading:
            ...
            if(previousState == CSSPullToRefreshStateTriggered && pullToRefreshActionHandler)
...
#pragma mark - CSSPullToRefreshArrow

@implementation CSSPullToRefreshArrow
