...
#import "UIImage+CSS.h"

double CSOnedayTimeIntervalValue = 24*60*60;  //一天的秒数
@implementation CSSSessionUtil
...
+ (NSString *)showNick:(NSString*)uid inSession:(NIMSession*)session{
    ...
    if (!nickname.length) {
        CSKitInfo *info = [[CSKit sharedKit] infoByUser:uid];
...
+(NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    result = [CSSSessionUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12)
    {
        hour = hour - 12;
    }
    ...
    else if([nowDate timeIntervalSinceDate:msgDate] < 7 * CSOnedayTimeIntervalValue)//一周内
    {
        NSString *weekDay = [CSSSessionUtil weekdayStr:msgDateComponents.weekday];
...
+ (NSDictionary *)dictByJsonString:(NSString *)jsonString
{
    ...
    return [CSSSessionUtil dictByJsonData:data];
}
