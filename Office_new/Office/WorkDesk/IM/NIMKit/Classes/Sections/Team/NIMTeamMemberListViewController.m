...
#define CollectionItemHeight 65
...
@property (nonatomic,strong) UIAlertView *kickAlertView;

@property (nonatomic,strong) NSString *kickMemberId;
...
- (void)cellDidSelected:(NIMTeamCardHeaderCell*)cell{
    ...
    _kickAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"将该用户移出本群？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [_kickAlertView show];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if (alertView == _kickAlertView) {
        if(alertView.cancelButtonIndex != buttonIndex) {
            [[NIMSDK sharedSDK].teamManager kickUsers:@[_kickMemberId] fromTeam:_team.teamId completion:^(NSError *error) {
                if(!error) {
