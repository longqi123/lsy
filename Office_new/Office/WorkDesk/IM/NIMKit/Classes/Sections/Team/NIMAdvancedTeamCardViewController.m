...
@property (nonatomic,strong) UIButton *editTitleBtn;
...
- (instancetype)initWithTeam:(NIMTeam*)team{
    ...
    if (self) {
        ...
        _avatar  = [[NIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 42, 42)];
        [_avatar addTarget:self action:@selector(onTouchAvatar:) forControlEvents:UIControlEventTouchUpInside];
        _backgroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 80)];
        _backgroundView.backgroundColor = UIColor.whiteColor;
        _editTitleBtn = [[UIButton alloc]init];
        [_editTitleBtn setImage:[UIImage imageNamed:@"xiu_gai_zi_liao"] forState:UIControlStateNormal];
        [_editTitleBtn sizeToFit];
        [_editTitleBtn addTarget:self action:@selector(updateTeamName) forControlEvents:UIControlEventTouchUpInside];
        ...
        [_backgroundView addSubview:_avatar];
        [_backgroundView addSubview:_titleLabel];
        [_backgroundView addSubview:_editTitleBtn];
//        [_backgroundView addSubview:_numberLabel];
//        [_backgroundView addSubview:_createTimeLabel];
...
- (void)updateTeamName
{
    if ([self.delegate respondsToSelector:@selector(updateTeamName)]) {
        [self.delegate updateTeamName];
    }
}
...
#define AvatarLeft 12
#define AvatarTop  25
#define TitleAndAvatarSpacing 15
...
- (void)layoutSubviews{
    ...
    self.avatar.nim_centerY  = _backgroundView.nim_centerY;
    self.titleLabel.nim_left = self.avatar.nim_right + TitleAndAvatarSpacing;
    self.titleLabel.nim_centerY  = _backgroundView.nim_centerY;
    ...
    self.editTitleBtn.nim_centerY = _backgroundView.nim_centerY;
    self.editTitleBtn.nim_right = _backgroundView.nim_right - AvatarLeft;
}
...
@interface NIMAdvancedTeamCardViewController ()<NIMAdvancedTeamMemberCellActionDelegate,NIMContactSelectDelegate,NIMTeamSwitchProtocol,NIMAdvancedTeamCardHeaderViewDelegate,NIMTeamManagerDelegate,UIActionSheetDelegate,UITableViewDataSource,UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>{
    ...
    NIMAdvancedTeamCardHeaderView *_headerView;
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _headerView = [[NIMAdvancedTeamCardHeaderView alloc] initWithTeam:self.team];
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"TeamCardTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"TeamCardTableViewCell"];
...
- (void)reloadData{
    ...
    if (self.myTeamInfo.type == NIMTeamMemberTypeOwner) {
        _headerView.editTitleBtn.hidden = false;
    }else{
        _headerView.editTitleBtn.hidden = true;
    }
}
...
- (UITableViewCell*)builidCommonCell:(id<NTESCardBodyData>) bodyData{
    TeamCardTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"TeamCardTableViewCell"];
    if (!cell) {
        cell = [[TeamCardTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"TeamCardTableViewCell"];
    }
    cell.detail.text = bodyData.subTitle;

    
    self.myTeamInfo = [[NIMSDK sharedSDK].teamManager teamMember:self.myTeamInfo.userId inTeam:self.myTeamInfo.teamId];

    if (self.myTeamInfo.type == NIMTeamMemberTypeOwner) {
        cell.editImageView.hidden = false;
    }else{
        cell.editImageView.hidden = true;
    }
    return cell;
}
