...
- (BOOL)onTapPressCell:(NIMMessage *)message inView:(UIView *)view{
#warning tyx
    NSString *ID = [message.remoteExt objectForKey:@"ID"];
    if (ID == nil) {
        return true;
    }else{
        [SVProgressHUD show];
        ...
        [[NIMSDK sharedSDK].chatroomManager enterChatroom:request completion:^(NSError *error, NIMChatroom *chatroom, NIMChatroomMember *me) {
            ...
            if (!error) {
                ...
                MeetingInviteViewController *vc = [[MeetingInviteViewController alloc]initWithNibName:@"MeetingInviteViewController" bundle:[NSBundle mainBundle]];
                [vc setDataSource:message];
                [self.navigationController pushViewController:vc animated:YES];
                
                NSMutableArray *vcs = [wself.navigationController.viewControllers mutableCopy];
                [vcs removeObject:self];
                wself.navigationController.viewControllers = vcs;
            }else {
