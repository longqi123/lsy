...
@interface CSSMeetingMessageHandler()<NIMChatManagerDelegate, NIMSystemNotificationManagerDelegate>

@property(nonatomic, strong) NIMChatroom *chatroom;

@property(nonatomic, weak) id<CSSMeetingMessageHandlerDelegate> delegate;

@end

@implementation CSSMeetingMessageHandler
...
- (void)dealCustomMessage:(NIMMessage *)message
{
    ...
    CSSMeetingControlAttachment *attachment  = (CSSMeetingControlAttachment *)object.attachment;
...
- (void)onMeetingCommand:(CSSMeetingControlAttachment *)attachment from:(NSString *)user
{
    if (![attachment.roomID isEqualToString:_chatroom.roomId]) {
        return;
    }
...
- (void)sendMeetingBroadcastCommand:(CSSMeetingControlAttachment *)attachment
{
    ...
    NIMMessage *message = [CSSSessionMsgConverter msgWithMeetingControlAttachment:attachment];
