//
//  CSInputEmoticonContainerView.m
//  CSKit
//
//  Created by chris.
...
#import "CSInputEmoticonContainerView.h"
#import "CSPageView.h"
#import "UIView+CS.h"
...
NSInteger CSCustomPageControlHeight = 36;
NSInteger CSCustomPageViewHeight    = 159;

@interface CSInputEmoticonContainerView()<NIMEmoticonButtonTouchDelegate,CSInputEmoticonTabDelegate>
...
@implementation CSInputEmoticonContainerView
...
{
    _emoticonPageView                  = [[CSPageView alloc] initWithFrame:self.bounds];
    _emoticonPageView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    _emoticonPageView.nim_height       = CSCustomPageViewHeight;
    ...
    _emotPageController = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 0, self.nim_width, CSCustomPageControlHeight)];
    ...
    _tabView = [[CSInputEmoticonTabView alloc] initWithFrame:CGRectMake(0, 0, self.nim_width, 0) catalogs:catalogs];
...
- (NSArray *)reloadData{
    CSInputEmoticonCatalog * defaultCatalog = [self loadDefaultCatalog];
...
- (NSInteger)sumPages
{
    ...
    [self.tabView.emoticonCatalogs enumerateObjectsUsingBlock:^(CSInputEmoticonCatalog* data, NSUInteger idx, BOOL *stop) {
...
- (UIView*)emojPageView:(CSPageView*)pageView inEmoticonCatalog:(CSInputEmoticonCatalog *)emoticon page:(NSInteger)page
{
    UIView *subView = [[UIView alloc] init];
    NSInteger iconHeight    = emoticon.layout.imageHeight;
    NSInteger iconWidth     = emoticon.layout.imageWidth;
    CGFloat startX          = (emoticon.layout.cellWidth - iconWidth) / 2  + CSKit_EmojiLeftMargin;
    CGFloat startY          = (emoticon.layout.cellHeight- iconHeight) / 2 + CSKit_EmojiTopMargin;
    ...
    {
        CSInputEmoticon *data = [emoticon.emoticons objectAtIndex:index];
        
        CSInputEmoticonButton *button = [CSInputEmoticonButton iconButtonWithData:data catalogID:emoticon.catalogID delegate:self];
        //计算表情位置
    ...
    if ([emoticon.catalogID isEqualToString:CSKit_EmojiCatalog]) {
...
- (void)addDeleteEmotButtonToView:(UIView *)view
                ...
                inEmoticonCatalog:(CSInputEmoticonCatalog *)emoticon
{
    CSInputEmoticonButton* deleteIcon = [[CSInputEmoticonButton alloc] init];
    ...
    NSString *prefix = [CSKit_EmoticonPath stringByAppendingPathComponent:CSKit_EmojiPath];
    ...
    UIImage *imageNormal  = [UIImage cs_imageInKit:imageNormalName];
    UIImage *imagePressed = [UIImage cs_imageInKit:imagePressName];
    ...
    CGRect deleteIconRect = CGRectMake(newX, newY, CSKit_DeleteIconWidth, CSKit_DeleteIconHeight);
...
{
    NSInteger page  = 0;
    CSInputEmoticonCatalog *emoticon;
...
- (CSInputEmoticonCatalog*)loadDefaultCatalog
{
    CSInputEmoticonCatalog *emoticonCatalog = [[CSInputEmoticonManager sharedManager] emoticonCatalog:CSKit_EmojiCatalog];
    if (emoticonCatalog) {
        CSInputEmoticonLayout *layout = [[CSInputEmoticonLayout alloc] initEmojiLayout:self.nim_width];
...
- (NSArray *)loadChartlet{
    NSArray *chatlets = [[CSInputEmoticonManager sharedManager] loadChartletEmoticonCatalog];
    for (CSInputEmoticonCatalog *item in chatlets) {
        CSInputEmoticonLayout *layout = [[CSInputEmoticonLayout alloc] initCharletLayout:self.nim_width];
        item.layout = layout;
...
//找到某组表情的起始位置
- (NSInteger)pageIndexWithEmoticon:(CSInputEmoticonCatalog *)emoticonCatalog{
    NSInteger pageIndex = 0;
    for (CSInputEmoticonCatalog *emoticon in self.tabView.emoticonCatalogs) {
...
- (NSInteger)pageIndexWithTotalIndex:(NSInteger)index{
    CSInputEmoticonCatalog *catelog = [self emoticonWithIndex:index];
    NSInteger begin = [self pageIndexWithEmoticon:catelog];
    return index - begin;
}

- (CSInputEmoticonCatalog *)emoticonWithIndex:(NSInteger)index {
    NSInteger page  = 0;
    CSInputEmoticonCatalog *emoticon;
    for (emoticon in self.tabView.emoticonCatalogs) {
        NSInteger newPage = page + emoticon.pagesCount;
...
- (NSInteger)numberOfPagesWithEmoticon:(CSInputEmoticonCatalog *)emoticonCatalog
{
...
- (void)pageViewScrollEnd: (CSPageView *)pageView
    ...
    CSInputEmoticonCatalog *emticon = [self emoticonWithIndex:index];
...
- (NSArray *)allEmoticons{
    ...
    for (CSInputEmoticonCatalog *catalog in self.tabView.emoticonCatalogs) {
