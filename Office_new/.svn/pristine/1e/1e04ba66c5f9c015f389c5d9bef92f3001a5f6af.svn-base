//
//  CSSChatroomMemberListViewController.m
//  NIM
//
...
@interface CSSChatroomMemberListViewController()<UITableViewDataSource,UITableViewDelegate,NIMChatManagerDelegate, CSSTimerHolderDelegate>
...
@property (nonatomic, strong) CSSTimerHolder *fetchMembersTimer;

@property (nonatomic, assign) BOOL fetchMembersCooldown;

@end

@implementation CSSChatroomMemberListViewController
...
- (instancetype)initWithChatroom:(NIMChatroom *)chatroom{
    ...
    if (self) {
        ...
        _fetchMembersTimer = [[CSSTimerHolder alloc] init];
    }
    return self;
}
...
- (void)viewDidLoad{
    ...
    [self.tableView registerClass:[CSSChatroomMemberCell class] forCellReuseIdentifier:self.cellReuseIdentifier];
        ...
        if (!error)
        {
            ...
            if (members.count == wself.limit)
            {
                [wself.tableView addPullToRefreshWithActionHandler:^{
                    [wself loadMoreData];
                } position:CSSPullToRefreshPositionBottom];
            }
            else
            {
...
{
    CSSChatroomMemberCell *cell = [tableView dequeueReusableCellWithIdentifier:self.cellReuseIdentifier];
    NIMChatroomMember *member = self.members[indexPath.row];
    [cell refresh:member];
    return cell;
}

#pragma mark - CSSTimerHolderDelegate
...
- (void)kickMemberAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[NIMSDK sharedSDK].chatroomManager kickMember:request completion:^(NSError *error) {
        if (!error)
        {
            ...
            if (idx != NSNotFound) {
                [[CSSMeetingRolesManager sharedInstance] kick:member.userId];
