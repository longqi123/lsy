...
#import "CSSessionListCell.h"
#import "UIView+CS.h"
#import "CSAvatarImageView.h"
#import "CSKitUtil.h"


@interface CSSessionListViewController ()<NIMConversationManagerDelegate,NIMTeamManagerDelegate,NIMUserManagerDelegate>

@end

@implementation CSSessionListViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onTeamInfoHasUpdatedNotification:) name:CSKitTeamInfoHasUpdatedNotification object:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CSSessionListCell * cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (!cell) {
        cell = [[CSSessionListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
...
- (void)onSelectedRecent:(NIMRecentSession *)recentSession atIndexPath:(NSIndexPath *)indexPath{
    CSSessionViewController *vc = [[CSSessionViewController alloc] initWithSession:recentSession.session];
...
- (NSString *)nameForRecentSession:(NIMRecentSession *)recent{
    if (recent.session.sessionType == NIMSessionTypeP2P) {
        return [CSKitUtil showNick:recent.session.sessionId inSession:recent.session];
    }else{
...
- (NSString *)timestampDescriptionForRecentSession:(NIMRecentSession *)recent{
    return [CSKitUtil showTime:recent.lastMessage.timestamp showDetail:NO];
}
...
- (NSString*)messageContent:(NIMMessage*)lastMessage{
    ...
    if (lastMessage.session.sessionType == NIMSessionTypeP2P) {
        ...
        NSString *nickName = [CSKitUtil showNick:lastMessage.from inSession:lastMessage.session];
