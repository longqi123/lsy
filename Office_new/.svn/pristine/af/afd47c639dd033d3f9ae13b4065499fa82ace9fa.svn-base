...
@interface CSSessionViewController ()
<UITableViewDataSource,
UITableViewDelegate,
NIMConversationManagerDelegate,
NIMTeamManagerDelegate,
NIMMediaManagerDelgate,
CSMessageCellDelegate,
NIMUserManagerDelegate>

@property (nonatomic,strong,readwrite) UITableView *tableView;

@property (nonatomic,strong) CSSessionMsgDatasource *sessionDatasource;
...
@implementation CSSessionViewController
...
- (void)makeUI
{
    ...
    CSCustomLeftBarView *leftBarView = [[CSCustomLeftBarView alloc] init];
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithCustomView:leftBarView];
    self.navigationItem.leftBarButtonItem = leftItem;
    self.navigationItem.leftItemsSupplementBackButton = YES;

    self.view.backgroundColor = [UIColor clearColor];
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height + [CSUIConfig topInputViewHeight]) style:UITableViewStylePlain];
    //self.tableView.backgroundColor = CSKit_UIColorFromRGB(0xe4e7ec);
    ...
    CGRect inputViewRect = CGRectMake(0, 0, self.view.nim_width, [CSUIConfig topInputViewHeight]);
    ...
    if (!disableInputView) {
        //_sessionInputView = [[CSInputView alloc] initWithFrame:inputViewRect];
...
{
    _layoutManager = [[CSSessionViewLayoutManager alloc] initWithInputView:self.sessionInputView tableView:self.tableView];
    
    //数据
    id<CSKitMessageProvider> dataProvider = [self.sessionConfig respondsToSelector:@selector(messageDataProvider)] ? [self.sessionConfig messageDataProvider] : nil;
    NSInteger limit = [CSUIConfig messageLimit];
    if ([self.sessionConfig respondsToSelector:@selector(messageLimit)]) {
        limit = self.sessionConfig.messageLimit;
    }
    NSTimeInterval showTimestampInterval = [CSUIConfig messageTimeInterval];
    ...
    _sessionDatasource = [[CSSessionMsgDatasource alloc] initWithSession:_session dataProvider:dataProvider showTimeInterval:showTimestampInterval limit:limit];
...
- (void)viewDidLayoutSubviews{
    ...
    if (isFirstLayout) {
        [self.tableView cs_scrollToBottom:NO];
    }
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([model isKindOfClass:[CSMessageModel class]]) {
        cell = [CSMessageCellMaker cellInTable:tableView
                                 forMessageMode:model];
        [(CSMessageCell *)cell setMessageDelegate:self];
        cell.backgroundColor = UIColor.clearColor;
    }
    else if ([model isKindOfClass:[CSTimestampModel class]])
    {
        cell = [CSMessageCellMaker cellInTable:tableView
                                   forTimeModel:model];
        cell.backgroundColor = UIColor.clearColor;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([modelInArray isKindOfClass:[CSMessageModel class]])
    {
        CSMessageModel *model = (CSMessageModel *)modelInArray;
    ...
    else if ([modelInArray isKindOfClass:[CSTimestampModel class]])
    {
...
{
    if ([message.session isEqual:_session]) {
        CSMessageModel *model = [self makeModel:message];
...
- (void)changeLeftBarBadge:(NSInteger)unreadCount{
    CSCustomLeftBarView *leftBarView = (CSCustomLeftBarView *)self.navigationItem.leftBarButtonItem.customView;
...
- (NSString *)sessionTitle
{
    ...
    switch (type) {
        ...
        case NIMSessionTypeP2P:{
            title = [CSKitUtil showNick:self.session.sessionId inSession:self.session];
        }
            break;
        default:
            break;
...
- (void)recordAudio:(NSString *)filePath didCompletedWithError:(NSError *)error {
    if(!error) {
        if ([self recordFileCanBeSend:filePath]) {
            [self sendMessage:[CSMessageMaker msgWithAudio:filePath]];
        }else{
...
- (void)onTapCell:(CSKitEvent *)message{}
...
{
    dispatch_async(CSMessageDataPrepareQueue(), ^{
...
- (void)layoutConfig:(CSMessageModel *)model{
    model.sessionConfig = self.sessionConfig;
    if (model.layoutConfig == nil)
    {
        id<CSCellLayoutConfig> layoutConfig = nil;
        ...
        if (!layoutConfig) {
            layoutConfig = [CSDefaultValueMaker sharedMaker].cellLayoutDefaultConfig;
        }
...
- (CSMessageModel *)makeModel:(NIMMessage *)message{
    CSMessageModel *model = [self findModel:message];
    if (!model) {
        model = [[CSMessageModel alloc] initWithMessage:message];
    }
    [self layoutConfig:model];
    return model;
}

- (CSMessageModel *)findModel:(NIMMessage *)message{
    CSMessageModel *model;
    for (CSMessageModel *item in self.sessionDatasource.modelArray.reverseObjectEnumerator.allObjects) {
        if ([item isKindOfClass:[CSMessageModel class]] && [item.message isEqual:message]) {
            model = item;
...
{
    __weak CSSessionViewLayoutManager *layoutManager = self.layoutManager;
    __weak typeof(self) wself = self;
...
- (void)checkInsert
{
    ...
    if (weakSelf.tableView.isDecelerating || weakSelf.tableView.isDragging)
    {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), CSMessageDataPrepareQueue(), ^{
...
dispatch_queue_t CSMessageDataPrepareQueue()
{
