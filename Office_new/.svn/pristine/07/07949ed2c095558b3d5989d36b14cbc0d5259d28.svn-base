...
@interface NIMMessageCell()<NIMPlayAudioUIDelegate,NIMMessageContentViewDelegate>{
    ...
    UITapGestureRecognizer *_tapPressGesture;
...
- (void)makeGesture{
    ...
    if ([self.reuseIdentifier  isEqual: @"NIMSessionTextContentView"]) {
        _tapPressGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapGesturePress:)];
        [self addGestureRecognizer:_tapPressGesture];
    }
...
- (void)tapGesturePress:(UIGestureRecognizer*)gestureRecognizer
{
    if ([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]]
        ) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(onTapPressCell:inView:)]) {
            [self.delegate onTapPressCell:self.model.message
                                    inView:_bubbleView];
        }
    }
}
