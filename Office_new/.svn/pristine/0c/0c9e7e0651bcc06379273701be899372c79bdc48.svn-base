...
@interface CSTeamMemberListViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,CSTeamCardHeaderCellDelegate, CSTeamMemberCardActionDelegate>
...
@implementation CSTeamMemberListViewController
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[CSTeamCardHeaderCell class] forCellWithReuseIdentifier:CollectionCellReuseId];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    CSTeamCardHeaderCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CollectionCellReuseId forIndexPath:indexPath];
    cell.delegate = self;
    id<CSKitCardHeaderData> data = [self dataAtIndexPath:indexPath];
    [cell refreshData:data];
    return cell;
}

- (id<CSKitCardHeaderData>)dataAtIndexPath:(NSIndexPath*)indexpath{
    NSInteger index = indexpath.section * self.collectionItemNumber;
    index += indexpath.row;
    return self.data[index];
}

- (NSIndexPath *)indexPathForData:(CSTeamCardMemberItem *)data{
...
- (void)cellDidSelected:(CSTeamCardHeaderCell*)cell{
    ...
    NSInteger index = indexpath.section * self.collectionItemNumber;
    index += indexpath.row;
    CSTeamMemberCardViewController *vc = [[CSTeamMemberCardViewController alloc] init];
    vc.delegate = self;
    CSTeamCardMemberItem *member = [[CSTeamCardMemberItem alloc] initWithMember:self.members[index]];
    CSTeamCardMemberItem *viewer = [[CSTeamCardMemberItem alloc] initWithMember:self.myTeamCard];
    vc.member = member;
    vc.viewer = viewer;
