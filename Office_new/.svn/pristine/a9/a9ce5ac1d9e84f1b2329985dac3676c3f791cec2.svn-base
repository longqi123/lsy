//
//  CSAdvancedTeamCardViewController.m
//  NIM
//
...
#import "CSAdvancedTeamCardViewController.h"
#import "CSTeamCardRowItem.h"
#import "UIView+CS.h"
#import "CSKitColorButtonCell.h"
#import "CSAdvancedTeamMemberCell.h"
#import "UIView+CSKitToast.h"
#import "CSTeamMemberCardViewController.h"
#import "CSCardMemberItem.h"
#import "CSContactSelectViewController.h"
#import "CSGroupedUsrInfo.h"
#import "CSTeamMemberListViewController.h"
#import "CSTeamAnnouncementListViewController.h"
#import "CSKitUtil.h"
#import "CSTeamSwitchTableViewCell.h"

#pragma mark - Team Header View
#define CardHeaderHeight 89
@interface CSAdvancedTeamCardHeaderView : UIView
...
@implementation CSAdvancedTeamCardHeaderView
...
- (instancetype)initWithTeam:(NIMTeam*)team{
    ...
    if (self) {
        ...
        _titleLabel.textColor            = CSKit_UIColorFromRGB(0x333333);
        _numberLabel                     = [[UILabel alloc]initWithFrame:CGRectZero];
        _numberLabel.backgroundColor     = [UIColor clearColor];
        _numberLabel.font                = [UIFont systemFontOfSize:14.f];
        _numberLabel.textColor           = CSKit_UIColorFromRGB(0x999999);
        ...
        _createTimeLabel.textColor       = CSKit_UIColorFromRGB(0x999999);
        [self addSubview:_avatar];
        [self addSubview:_titleLabel];
        [self addSubview:_numberLabel];
        [self addSubview:_createTimeLabel];
        
        self.backgroundColor = CSKit_UIColorFromRGB(0xecf1f5);
...
@interface CSAdvancedTeamCardViewController ()<CSAdvancedTeamMemberCellActionDelegate,NIMContactSelectDelegate,NIMTeamSwitchProtocol,UIActionSheetDelegate,UITableViewDataSource,UITableViewDelegate>{
...
@implementation CSAdvancedTeamCardViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    CSAdvancedTeamCardHeaderView *headerView = [[CSAdvancedTeamCardHeaderView alloc] initWithTeam:self.team];
    ...
    self.tableView.backgroundColor = CSKit_UIColorFromRGB(0xecf1f5);
...
- (void)reloadData{
    ...
    CSAdvancedTeamCardHeaderView *headerView = (CSAdvancedTeamCardHeaderView*)self.tableView.tableHeaderView;
...
- (void)requestData:(void(^)(NSError *error)) handler{
    ...
    [[NIMSDK sharedSDK].teamManager fetchTeamMembers:self.team.teamId completion:^(NSError *error, NSArray *members) {
        if (!error) {
            ...
            [wself.view CSKit_makeToast:@"你已经不在群里" duration:2
                                position:CSKitToastPositionCenter];
        }else{
            [wself.view CSKit_makeToast:[NSString stringWithFormat:@"拉好友失败 error: %zd",error.code] duration:2
                                position:CSKitToastPositionCenter];
        }
        handler(error);
    }];
...
- (void)buildBodyData{
    ...
    CSTeamCardRowItem *teamMember = [[CSTeamCardRowItem alloc] init];
    ...
    CSTeamCardRowItem *teamName = [[CSTeamCardRowItem alloc] init];
    teamName.title = @"群名称";
...
//    CSTeamCardRowItem *teamNick = [[CSTeamCardRowItem alloc] init];
//    teamNick.title = @"群昵称";
...
//    teamNick.type   = TeamCardRowItemTypeCommon;
    ...
    CSTeamCardRowItem *teamIntro = [[CSTeamCardRowItem alloc] init];
    teamIntro.title = @"群介绍";
...
//    CSTeamCardRowItem *teamAnnouncement = [[CSTeamCardRowItem alloc] init];
...
//    CSTeamCardRowItem *teamNotify = [[CSTeamCardRowItem alloc] init];
...
//    CSTeamCardRowItem *itemAuth = [[CSTeamCardRowItem alloc] init];
//    itemAuth.title  = @"身份验证";
...
- (NSIndexPath *)cellIndexPathByTitle:(NSString *)title {
    ...
    [self.bodyData enumerateObjectsUsingBlock:^(NSArray *rows, NSUInteger s, BOOL *stop) {
        ...
        [rows enumerateObjectsUsingBlock:^(CSTeamCardRowItem *item, NSUInteger r, BOOL *stop) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([bodyData respondsToSelector:@selector(action)]) {
        if (bodyData.action) {
            CSKit_SuppressPerformSelectorLeakWarning([self performSelector:bodyData.action]);
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CSKitTeamCardRowItemType type = bodyData.type;
    switch (type) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    view.backgroundColor = CSKit_UIColorFromRGB(0xecf1f5);
    return view;
}
...
- (UITableViewCell*)builidCommonCell:(id<NTESCardBodyData>) bodyData{
    ...
    if (!cell) {
        ...
        sep.backgroundColor = CSKit_UIColorFromRGB(0xebebeb);
        [cell addSubview:sep];
...
- (UITableViewCell*)builidRedButtonCell:(id<NTESCardBodyData>) bodyData{
    CSKitColorButtonCell * cell = [self.tableView dequeueReusableCellWithIdentifier:TableButtonCellReuseId];
    if (!cell) {
        cell = [[CSKitColorButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TableButtonCellReuseId];
    }
    cell.button.style = CSKitColorButtonCellStyleRed;
...
- (UITableViewCell*)builidTeamMemberCell:(id<NTESCardBodyData>) bodyData{
    CSAdvancedTeamMemberCell * cell = [self.tableView dequeueReusableCellWithIdentifier:TableMemberCellReuseId];
    if (!cell) {
        cell = [[CSAdvancedTeamMemberCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:TableMemberCellReuseId];
        cell.delegate = self;
    }
...
{
    CSTeamSwitchTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:TableSwitchReuseId];
    if (!cell) {
        cell = [[CSTeamSwitchTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CSTeamSwitchTableViewCell"];
    }
    cell.textLabel.text = bodyData.title;
...
- (void)updateTeamAnnouncement{
    BOOL isManager = self.myTeamInfo.type == NIMTeamMemberTypeManager || self.myTeamInfo.type == NIMTeamMemberTypeOwner;
    CSTeamAnnouncementListViewController *vc = [[CSTeamAnnouncementListViewController alloc] initWithNibName:nil bundle:nil];
    vc.team = self.team;
...
- (void)enterMemberCard{
    CSTeamMemberListViewController *vc = [[CSTeamMemberListViewController alloc] initTeam:self.team members:self.memberData];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)onStateChanged:(BOOL)on
{
    ...
    [[[NIMSDK sharedSDK] teamManager] updateNotifyState:on
                                             ...
                                             completion:^(NSError *error) {
                                                 if (error) {
                                                     [weakSelf.view CSKit_makeToast:[NSString stringWithFormat:@"修改失败  error:%zd",error.code]
                                                                            duration:2
                                                                            position:CSKitToastPositionCenter];
                                                 }
                                                 [weakSelf reloadData];
...
#pragma mark - CSAdvancedTeamMemberCellActionDelegate
...
- (void)didSelectAddOpeartor{
    ...
    CSContactFriendSelectConfig *config = [[CSContactFriendSelectConfig alloc] init];
    config.filterIds = users;
    config.needMutiSelected = YES;
    CSContactSelectViewController *vc = [[CSContactSelectViewController alloc] initWithConfig:config];
    vc.delegate = self;
    [vc show];
}
...
- (void)didFinishedSelect:(NSArray *)selectedContacts{
    ...
    [[NIMSDK sharedSDK].teamManager addUsers:selectedContacts toTeam:self.team.teamId postscript:postscript completion:^(NSError *error, NSArray *members) {
        if (!error) {
            [self.view CSKit_makeToast:@"邀请成功"
                               duration:2
                               position:CSKitToastPositionCenter];
        }else{
            [self.view CSKit_makeToast:[NSString stringWithFormat:@"邀请失败 code:%zd",error.code]
                               duration:2
                               position:CSKitToastPositionCenter];
        }
    }];
            ...
            if (name.length) {
                ...
                [[NIMSDK sharedSDK].teamManager updateTeamName:name teamId:self.team.teamId completion:^(NSError *error) {
                    if (!error) {
                        ...
                        [self.view CSKit_makeToast:@"修改成功" duration:2
                                           position:CSKitToastPositionCenter];
                        [self reloadData];
                    }else{
                        [self.view CSKit_makeToast:[NSString stringWithFormat:@"修改失败 code:%zd",error.code] duration:2
                                           position:CSKitToastPositionCenter];
                    }
                }];
            }
            ...
            if (name.length) {
                ...
                [[NIMSDK sharedSDK].teamManager updateUserNick:currentUserId newNick:name inTeam:self.team.teamId completion:^(NSError *error) {
                    if (!error) {
                        ...
                        [self.view CSKit_makeToast:@"修改成功"];
                        [self reloadData];
                    }else{
                        [self.view CSKit_makeToast:[NSString stringWithFormat:@"修改失败 code:%zd",error.code]];
                    }
                }];
            }
            break;
        ...
        case 1:{
            [[NIMSDK sharedSDK].teamManager quitTeam:self.team.teamId completion:^(NSError *error) {
                if (!error) {
                    ...
                    [self.view CSKit_makeToast:[NSString stringWithFormat:@"退出失败 code:%zd",error.code]];
                }
            }];
            break;
        }
        ...
        case 1:{
            [[NIMSDK sharedSDK].teamManager dismissTeam:self.team.teamId completion:^(NSError *error) {
                if (!error) {
                    ...
                    [self.view CSKit_makeToast:[NSString stringWithFormat:@"解散失败 code:%zd",error.code]];
                }
            }];
            break;
        }
    ...
    if (actionSheet == _moreActionSheet) {
        ...
        __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber){
            [[NIMSDK sharedSDK].teamManager transferManagerWithTeam:wself.team.teamId newOwnerId:memeber.firstObject isLeave:isLeave completion:^(NSError *error) {
                if (!error) {
                    [wself.view CSKit_makeToast:@"转移成功！" duration:2.0 position:CSKitToastPositionCenter];
                    if (isLeave) {
                    ...
                    [wself.view CSKit_makeToast:[NSString stringWithFormat:@"转移失败！code:%zd",error.code] duration:2.0 position:CSKitToastPositionCenter];
                }
            }];
        };
        NSString *currentUserID = [[[NIMSDK sharedSDK] loginManager] currentAccount];
        CSContactTeamMemberSelectConfig *config = [[CSContactTeamMemberSelectConfig alloc] init];
