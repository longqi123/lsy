...
#import "CSNormalTeamCardViewController.h"
...
@interface CSNormalTeamCardViewController ()<NIMTeamManagerDelegate, CSTeamMemberCardActionDelegate,UITableViewDataSource,UITableViewDelegate,NIMTeamSwitchProtocol,NIMContactSelectDelegate,CSMemberGroupViewDelegate>{
...
@property (nonatomic,strong) CSMemberGroupView *headerView;

@property (nonatomic,strong) UITableView *tableView;

@property (nonatomic,assign) CSKitCardHeaderOpeator currentOpera;
...
@implementation CSNormalTeamCardViewController
    ...
    [self requestData:^(NSError *error, NSArray *data) {
        if (!error) {
           [wself refreshWithMembers:data];
            ...
            [wself.view CSKit_makeToast:@"讨论组成员获取失败"];
        }
    ...
    [[NIMSDK sharedSDK].teamManager fetchTeamMembers:self.team.teamId completion:^(NSError *error, NSArray *members) {
        ...
        if (!error) {
            ...
            for (NIMTeamMember *member in members) {
                CSTeamCardMemberItem *item = [[CSTeamCardMemberItem alloc] initWithMember:member];
                [array addObject:item];
            }
            wself.teamMembers = members;
        }else if(error.code == NIMRemoteErrorCodeTeamNotMember){
            [wself.view CSKit_makeToast:@"你已经不在讨论组里"];
        }else{
            [wself.view CSKit_makeToast:@"拉好友失败"];
        }
        handler(error,array);
    }];
}
...
- (NSArray*)buildBodyData{
    ...
    CSTeamCardRowItem *itemName = [[CSTeamCardRowItem alloc] init];
                ...
                [[NIMSDK sharedSDK].teamManager addUsers:selectedContacts
                    ...
                    if (!error) {
                        if (self.team.type == NIMTeamTypeNormal) {
                            ...
                            [wself.view CSKit_makeToast:@"邀请成功，等待验证" duration:2.0 position:CSKitToastPositionCenter];
                        }
                        [wself refreshTableHeader:self.view.nim_width];
                    }else{
                        [wself.view CSKit_makeToast:@"邀请失败"];
                    }
...
- (void)transferOwner:(NSString *)memberId isLeave:(BOOL)isLeave{
    ...
    [[NIMSDK sharedSDK].teamManager transferManagerWithTeam:self.team.teamId newOwnerId:memberId isLeave:isLeave completion:^(NSError *error) {
        if (!error) {
            ...
            [wself.view CSKit_makeToast:@"修改成功"];
        }else{
            [wself.view CSKit_makeToast:@"修改失败"];
        }
    }];
}
...
- (UITableViewCell*)builidBlueButtonCell:(id<NTESCardBodyData>) bodyData indexPath:(NSIndexPath *)indexPath{
    ...
    cell.button.style = CSKitColorButtonCellStyleBlue;
    [cell.button setTitle:bodyData.title forState:UIControlStateNormal];
    return cell;
}
                ...
                if (name.length) {
                    [[NIMSDK sharedSDK].teamManager updateTeamName:name teamId:self.team.teamId completion:^(NSError *error) {
                        if (!error) {
                            ...
                            [self.view CSKit_makeToast:@"修改失败"];
                        }
                    }];
                }
                break;
            ...
            case 1:{
                [[NIMSDK sharedSDK].teamManager quitTeam:self.team.teamId completion:^(NSError *error) {
                    if (!error) {
                        ...
                        [self.view CSKit_makeToast:@"退出失败"];
                    }
                }];
                break;
            }
...
#pragma mark - CSMemberGroupViewDelegate
- (void)didSelectRemoveButtonWithMemberId:(NSString *)uid{
    ...
    [[NIMSDK sharedSDK].teamManager kickUsers:@[uid] fromTeam:self.team.teamId completion:^(NSError *error) {
        ...
        if (!error) {
            ...
            [wself.view CSKit_makeToast:@"移除失败"];
        }
    }];
}
...
- (void)setupTableHeader:(CGFloat)width{
    self.headerView = [[CSMemberGroupView alloc] initWithFrame:CGRectZero];
    self.headerView.delegate = self;
    CSKitCardHeaderOpeator opeartor;
...
- (NSArray*)headerUserIds{
    ...
    for (id<CSKitCardHeaderData> data in self.headerData) {
...
- (void)addMembers:(NSArray*)members{
    ...
    for (id<CSKitCardHeaderData> data in self.headerData.reverseObjectEnumerator.allObjects) {
