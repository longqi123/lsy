...
static char CSimageURLKey;


@interface CSAvatarImageView()
@end

@implementation CSAvatarImageView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (image && image.size.height && image.size.width)
    {
        ...
        CGRect  drawRect = CSKit_CGRectWithCenterAndSize(center, size);
...
CGRect CSKit_CGRectWithCenterAndSize(CGPoint center, CGSize size){
...
- (void)setAvatarBySession:(NIMSession *)session
{
    ...
    {
        info = [[CSKit sharedKit] infoByTeam:session.sessionId];
    }
    else
    {
        info = [[CSKit sharedKit] infoByUser:session.sessionId
                                    inSession:session];
    }
...
{
    CSKitInfo *info = [[CSKit sharedKit] infoByUser:message.from
                                          withMessage:message];
    NSURL *url = info.avatarUrlString ? [NSURL URLWithString:info.avatarUrlString] : nil;
    [self nim_setImageWithURL:url placeholderImage:info.avatarImage];
}
...
@implementation CSAvatarImageView (SDWebImageCache)
...
- (void)nim_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(NIMWebImageOptions)options progress:(NIMWebImageDownloaderProgressBlock)progressBlock completed:(NIMWebImageCompletionBlock)completedBlock {
    ...
    objc_setAssociatedObject(self, &CSimageURLKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (NSURL *)nim_imageURL {
    return objc_getAssociatedObject(self, &CSimageURLKey);
}
